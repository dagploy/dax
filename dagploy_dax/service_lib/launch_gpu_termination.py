...
class LaunchGPUTermination:
    ...
    def log_config(self) -> None:
        hidden_keys = ["id", "token", "secret", "user", "account", "default"]

        self.on_output(" ⌨  Run configuration ...")

        for k, v in self.merged.items():
            if not any(x in k.lower() for x in hidden_keys):
        ...
        for idx, zone in enumerate(zones, start=1):
            ...
            try:
                ...
                if is_resource_unavailable_error(e):
                    self.on_output(
                        f"⤫ [bold red]GPU/resource unavailable in {zone}. "
                        "Trying next zone...[/bold red]"
                    )
                    continue
    ...
    def __call__(self):
        ...
        outs, vm_name, external_ip = self.validate_outputs(stack_name, result)
        ...
        if external_ip:
            self.on_output(f"🚀 [bold green]External IP: http://{external_ip}[/bold green]")
